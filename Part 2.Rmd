...
```{r}
ggplot(finalP) + geom_bar(mapping = aes(format(finalP$Submit_Date,"%m"), na.rm = T), na.rm = T)
ggplot(finalP) + geom_bar(mapping = aes(format(finalP$Determin_Date,"%m"), na.rm = T))
...
ggplot(data = salD, mapping = aes(x = SUPERVISE_OTHER_EMP, y = sal)) +  geom_col()
```
```{r}
...
ggplot(data = finalP, mapping = aes(x = SUPERVISE_HOW_MANY, y = PWD_WAGE_RATE)) +  geom_point() +  geom_smooth(se = TRUE, method = "lm") + xlab("Employees Supervised") + ylab("Mean Salary") + ggtitle("Salary vs Number of Employees Supervised") +xlim(0, 375)+ylim(0, 300000)
```
